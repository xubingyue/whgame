...
void CManagerSendWarning::OnOK()
{
	...
	if ((m_pIUserItem==NULL)||(m_pIUserItem->GetUserID()!=m_dwUserID))
	{
		...
		{
			AfxMessageBox(TEXT("目标用户已经离开了游戏房间，无法向此用户发送警告通知！"),MB_ICONQUESTION);
			return;
		}
	}
	...
	SendWarning.wChatLength=CountString(SendWarning.szWarningMessage);

	//消息判断
	if (SendWarning.szWarningMessage[0]==0)
	{
		AfxMessageBox(TEXT("警告消息不能为空，请写入警告消息内容！"),MB_ICONQUESTION);
...
{
	//查询接口
	m_pIUserItem=GET_OBJECTPTR_INTERFACE(pIUnknownEx,IUserItem);
...
{
	//查询接口
	m_pIClientSocket=GET_OBJECTPTR_INTERFACE(pIUnknownEx,IClientSocket);
...
{
	//查询接口
	m_pIClientUserManager=GET_OBJECTPTR_INTERFACE(pIUnknownEx,IClientUserManager);
...
void CManagerSendWarning::OnPreViewMessage()
{
	...
	//预览消息
	MessageBox(szMessage,TEXT("警告预览："),MB_ICONQUESTION);
	m_WarningMessage.SetFocus();
...
void CManagerUserRight::OnOK()
{
	...
	if ((m_pIUserItem==NULL)||(m_pIUserItem->GetUserID()!=m_dwUserID))
	{
		...
		if (m_pIUserItem==NULL)
		{
			AfxMessageBox(TEXT("目标用户已经离开了游戏房间，用户权限设置失败！"),MB_ICONQUESTION);
			return;
		}
	}
...
void CManagerMessage::OnOK()
{
	...
	SendMessage.wChatLength=CountString(SendMessage.szSystemMessage);

	//消息判断
	if (SendMessage.szSystemMessage[0]==0)
	{
		AfxMessageBox(TEXT("系统消息不能为空，请写入系统消息内容！"),MB_ICONQUESTION);
	...
	{
		AfxMessageBox(TEXT("发送失败，请选择系统消息的类型！"),MB_ICONQUESTION);
		return;
	}
...
void CManagerMessage::OnPreViewMessage()
{
	...
	//预览消息
	MessageBox(szMessage,TEXT("消息预览："),MB_ICONQUESTION);
	m_SystemMessage.SetFocus();
