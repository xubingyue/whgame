...
void CGameFrameDlg::EnableSound(bool bEnable)
{
	...
	//控制按钮
	UINT uImageID=m_bAllowSound?IDB_BT_SOUND:IDB_BT_SOUND_EX;
	m_pGameFrameControl->m_btSound.SetButtonImage(uImageID,GetModuleHandle(GAME_FRAME_DLL_NAME),false);
...
bool CGameFrameDlg::AllowUserLookon(DWORD dwUserID, bool bAllowLookon)
{
	...
	{
		//控制按钮
		UINT uImageID=bAllowLookon?IDB_BT_LOOKON:IDB_BT_LOOKON_EX;
		m_pGameFrameControl->m_btLookOn.SetButtonImage(uImageID,GetModuleHandle(GAME_FRAME_DLL_NAME),false);
	}
...
BOOL CGameFrameDlg::OnInitDialog()
{
	...
	m_btControl.SetButtonImage(IDB_HIDE_USER_LIST,hInstance,false);
	if (IsEnableSound()==false) m_pGameFrameControl->m_btSound.SetButtonImage(IDB_BT_SOUND_EX,hInstance,false);
	if (IsAllowUserLookon()==false) m_pGameFrameControl->m_btLookOn.SetButtonImage(IDB_BT_LOOKON_EX,hInstance,false);
...
VOID CGameFrameDlg::OnBnClickedControl()
{
	...
	//设置界面
	HINSTANCE hInstance=GetModuleHandle(GAME_FRAME_DLL_NAME);
	m_btControl.SetButtonImage(m_bHideList?IDB_HIDE_USER_LIST:IDB_SHOW_USER_LIST,hInstance,false);
