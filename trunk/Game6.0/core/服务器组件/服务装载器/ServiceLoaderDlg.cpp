...
BOOL CServiceLoaderDlg::OnInitDialog()
{
	...
	{
		m_RichEditCtrl.InsertString(TEXT("【系统提示】：“事件服务”创建失败或者组件不存在\n"),RGB(200,0,0),false);
	}
...
void CServiceLoaderDlg::OnCancel()
{
	...
	if (m_GameService.GetInterface()!=NULL)
	{
		...
		//停止服务
		if (bRuning==true) m_GameService->EndService();
	}
...
void CServiceLoaderDlg::OnBnClickedStart()
{
	...
	{
		ShowErrorMessasge(TEXT("游戏数据库信息不存在，游戏房间服务启动失败"),Level_Exception);
		return;
	}
	...
	if (m_GameService.GetInterface()==NULL)
	{
		...
		{
			ShowErrorMessasge(TEXT("游戏服务组件创建失败"),Level_Exception);
			return;
		}
	}
	...
	//启动服务
	TCHAR szDescribe[128]=TEXT("");
	if (m_GameService->BeginService(&GameServiceParameter)==true)
	{
		//界面控制
...
{
	if ((m_GameService.GetInterface()!=NULL)&&(m_GameService->EndService()==true))
	{
		...
		GetDlgItem(IDC_OPTION)->EnableWindow(TRUE);
		ShowErrorMessasge(TEXT("游戏房间服务停止成功"),Level_Normal);
	}
...
void CServiceLoaderDlg::OnBnClickedCreate()
{
	...
	if (bSuccess==true)	ShowErrorMessasge(TEXT("游戏房间创建成功"),Level_Normal);
...
void CServiceLoaderDlg::OnBnClickedSaveOption()
{
	...
	{
		ShowErrorMessasge(TEXT("房间配置文件保存失败"),Level_Exception);
		return;
	}
...
const tagDBConnectInfo * CServiceLoaderDlg::GetDBConnectInfo(DWORD dwDataBaseAddr)
{
	...
	{
		ShowErrorMessasge(TEXT("数据库组件创建失败，无法获取数据库信息"),Level_Exception);
		return NULL;
	}
	...
	try
	{
		...
		{
			ShowErrorMessasge(TEXT("信息数据库连接失败，无法获取数据库信息"),Level_Exception);
			return NULL;
		}
		...
		ServerInfoDB->SetSPName("GSP_LoadDataBaseInfo");
		...
		ServerInfoDB->GetFieldValue(TEXT("DBAddr"),szDBAddr,sizeof(szDBAddr));
		ServerInfoDB->GetFieldValue(TEXT("DBUser"),szDBUser,sizeof(szDBUser));
		ServerInfoDB->GetFieldValue(TEXT("DBPort"),DBConnectInfo.wDataBasePort);
		ServerInfoDB->GetFieldValue(TEXT("DBPassword"),szDBPass,sizeof(szDBPass));
