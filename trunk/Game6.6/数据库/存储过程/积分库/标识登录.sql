...
BEGIN
	...
	-- 房间锁定
	IF EXISTS (SELECT UserID FROM GameTreasureDBLink.GameTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID)
	BEGIN
		SELECT [ErrorDescribe]=N'您已经在充值游戏房间了，不能同时在进入此游戏房间了！'
		RETURN 4
	END
	INSERT GameTreasureDBLink.GameTreasureDB.dbo.GameScoreLocker (UserID,KindID,ServerID) VALUES (@dwUserID,@wKindID,@wServerID)
	...
	-- 道具统计
	SELECT @PropCount=COUNT(UserID) FROM GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf WHERE UserID=@UserID AND UsedFlag=0 AND (KindID=@wKindID OR KindID=0)
	...
	BEGIN 
		...
		-- 删除会员属性
		DELETE FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@UserID
	END
	ELSE 
	BEGIN
		...
		FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@UserID

		-- 删除过期时间
		IF GETDATE()>=@MemberCurDate
		BEGIN
			-- 删除会员期限过期的所有会员
			DELETE FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@UserID AND MemberOverDate<=GETDATE() 
	...
	-- 更新信息
	UPDATE GameUserDBLink.GameUserDB.dbo.AccountsInfo SET MemberOrder=@MemberOrder WHERE UserID=@UserID

	-- 会员道具
	SELECT @PropCount=@PropCount+COUNT(UserID) FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@UserID
	
	-- 玩家金币
	SELECT @GameGold=Score,@InsureScore=InsureScore FROM GameTreasureDBLink.GameTreasureDB.dbo.GameScoreInfo WHERE UserID=@UserID
