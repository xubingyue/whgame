...
-- 执行逻辑
BEGIN
	INSERT GameTreasureDBLink.GameTreasureDB.dbo.RecordPurchaseProp(SendUserID,RcvUserID,KindID,ServerID,CateID,CurCount,OnceCount,PachurseCount,SpendScore,ClientIP)
	...
	BEGIN
		...
		FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@dwRcvUserID

		IF @MemberCount IS NULL OR @MemberCount<=0
		BEGIN
			INSERT GameUserDBLink.GameUserDB.dbo.MemberInfo (UserID,MemberOrder,MemberOverDate)
		...
		BEGIN	
			...
			FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@dwRcvUserID AND MemberOrder>=@curMemberOrder
			IF @UpMemberOverDate IS NULL SET @UpMemberOverDate=GETDATE()		
			
			UPDATE GameUserDBLink.GameUserDB.dbo.MemberInfo SET MemberOverDate=DATEADD(ss, @dwOnceCount*@dwPachurseCount, @UpMemberOverDate)
			...
			UPDATE GameUserDBLink.GameUserDB.dbo.MemberInfo SET MemberOverDate=DATEADD(ss, @dwOnceCount*@dwPachurseCount, MemberOverDate)
			WHERE UserID=@dwRcvUserID AND MemberOrder<@curMemberOrder
			--------------------------------------------------------------
		END		
		...
		-- 附加会员信息
		UPDATE GameUserDBLink.GameUserDB.dbo.AccountsInfo
...
BEGIN
	...
	FROM GameUserDBLink.GameUserDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN
		...
		FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@dwUserID	
		...
		BEGIN
			...
			FROM GameUserDBLink.GameUserDB.dbo.MemberInfo
			...
			FROM GameUserDBLink.GameUserDB.dbo.MemberInfo WHERE UserID=@dwUserID AND MemberOrder<@MaxMemberOrder
	...
	IF @dwKindID=0
		...
		IF GETDATE()<@MemberOverDate
		BEGIN
			SELECT UserID,CateID,KindID,PropCount FROM GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND UsedFlag=0
			UNION
	...
	ELSE
		...
		IF GETDATE()<@MemberOverDate
		BEGIN
			SELECT UserID,CateID,KindID,PropCount FROM GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND UsedFlag=0 AND (KindID=@dwKindID OR KindID=0)
			UNION
			SELECT UserID,CateID,KindID,PropCount FROM @tbMember
		END
		ELSE
...
BEGIN
	...
	IF @dwPropCount<=0
	BEGIN
		DELETE FROM GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf WHERE UserID=@dwUserID AND KindID=@dwKindID AND CateID=@dwCateID
	END
	ELSE
	BEGIN
		UPDATE GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf SET PropCount=@dwPropCount,UsedFlag=0,ClientIP=@strClientIP WHERE UserID=@dwUserID AND KindID=@dwKindID AND CateID=@dwCateID

		IF @@ROWCOUNT=0
		BEGIN
			INSERT GameTreasureDBLink.GameTreasureDB.dbo.GameUserShelf(UserID,CateID,KindID,PropCount,ClientIP) 
			VALUES(@dwUserID,@dwCateID,@dwKindID,@dwPropCount,@strClientIP)
		END
	END

	-- 写记录
	INSERT GameTreasureDBLink.GameTreasureDB.dbo.RecordUserProp (UserID,KindID,CateID,PropCount,ClientIP)
...
-- 执行逻辑
BEGIN
	SELECT [ID],[Name],Price,SendUserCharm,RcvUserCharm,Discount FROM GameTreasureDBLink.GameTreasureDB.dbo.FlowerCate WHERE Nullity=0
...
BEGIN
  ...
  FROM GameTreasureDBLink.GameTreasureDB.dbo.[GameShopCate] WHERE  Nullity=0
