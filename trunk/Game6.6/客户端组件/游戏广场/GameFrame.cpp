	...
	case ItemGenre_Inside:
		{
			...
			WebBrowse(TEXT("http://www.sxjingji.cn"),false);
	...
	case ItemGenre_Kind:		//游戏类型
		{
			...
			_snprintf(szRuleUrl,sizeof(szRuleUrl),TEXT("http://www.sxjingji.cn/GameRule.asp?KindID=%ld"),pGameKind->wKindID);
...
BOOL CGameFrame::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_BT_BUTTON_5:				//功能按钮
		{
			if(IDC_BT_BUTTON_1==nCommandID)WebBrowse(TEXT("http://www.sxjingji.cn/index.asp"),true);
			if(IDC_BT_BUTTON_2==nCommandID)WebBrowse(TEXT("http://www.sxjingji.cn/PayBuy.asp"),true);
			if(IDC_BT_BUTTON_4==nCommandID)WebBrowse(TEXT("http://www.sxjingji.cn/user/ExChange.asp"),true);
			if(IDC_BT_BUTTON_5==nCommandID)WebBrowse(TEXT("http://www.sxjingji.cn/bbs/index.asp?action=frameon"),true);
...
int CGameFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_BrowerAD.Navigate(TEXT("http://www.sxjingji.cn/AD/GamePlazaAD.asp"),NULL,NULL,NULL,NULL);
