...
void CCardControl::OnPaint()
{
	...
	for (INT i=0;i<m_CardDataItem.GetCount();i++)
	{
		...
		//绘画扑克
		if(m_dwSelected[i]){
			m_ImageCardSel.AlphaDrawImage( &BufferDC, dwXScreenPos, dwYScreenPos,m_CardSize.cx, m_CardSize.cy, dwXImagePos, dwYImagePos, 
			RGB( 255, 0, 255 ) );
		}else
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	//清空选中的牌
	memset(&m_dwSelected,0x00,sizeof(m_dwSelected));
...
void CCardControl::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_dwCurrentIndex!=0xFFFFFFFF){
		m_dwSelected[m_dwCurrentIndex] = TRUE;
	}
...
void CCardControl::OnMouseMove(UINT nFlags, CPoint point){
	...
	//设置控件
	m_dwSelected[dwCurrentIndex] = TRUE;
