	...
	case IDI_CALL_SCORE:	//用户叫分
		{
			...
			//设置变量
			CallScore.bLandScore=m_GameLogic.LandScore(m_pIAndroidUserItem->GetChairID(), m_cbCurrentLandScore, m_cbCallScorePhase);
...
bool __cdecl CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, void * pData, WORD wDataSize)
{
	...
	case SUB_S_DOUBLE_SCORE:	//加倍消息
		{
			return OnSubDoubleScore(pData,wDataSize);
		}
...
bool CAndroidUserItemSink::OnSubCallScore(void * pData, WORD wDataSize)
{
	...
	//设置变量
	m_cbCallScorePhase = pCallScore->bCallScorePhase;
...
//加倍消息
bool CAndroidUserItemSink::OnSubDoubleScore(void * pData, WORD wDataSize)
{
	//效验数据
	...
	//变量定义
	CMD_S_DoubleScore *pDoubleScore = ( CMD_S_DoubleScore * )pData;

	//设置底牌
	m_GameLogic.SetBackCard(pDoubleScore->wLandUser, pDoubleScore->bBackCard, BACK_COUNT) ;

	if (pDoubleScore->wLandUser==m_pIAndroidUserItem->GetChairID())
	{
		//设置扑克
		m_cbHandCardCount+=CountArray(pDoubleScore->bBackCard);
		CopyMemory(&m_cbHandCardData[NORMAL_COUNT],pDoubleScore->bBackCard,sizeof(pDoubleScore->bBackCard));
	...
	DoubleScore.bDoubleScore = false;

	//发送消息
	m_pIAndroidUserItem->SendSocketData( SUB_C_DOUBLE_SCORE, &DoubleScore, sizeof( DoubleScore ) );
