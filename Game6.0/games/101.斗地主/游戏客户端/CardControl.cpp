...
CSkinImage				CCardControl::m_ImageCardSel;					//图片资源
...
CCardControl::CCardControl()
{
	...
	if (m_bLoad==false)
	{
		...
		m_ImageCardSel.SetLoadInfo(IDB_CARD_SELECTED,AfxGetInstanceHandle());
...
void CCardControl::OnPaint()
{
	...
	CImageHandle HandleCardSel(&m_ImageCardSel);
	...
	for (INT i=0;i<m_CardDataItem.GetCount();i++)
	{
		...
		//绘画扑克
		if(m_bSelected[i]){
			m_ImageCardSel.AlphaDrawImage( &BufferDC, dwXScreenPos, dwYScreenPos,m_CardSize.cx, m_CardSize.cy, dwXImagePos, dwYImagePos, RGB( 255, 0, 255 ) );
		}else{
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	//清空选中的牌
	memset(&m_bSelected,0x00,sizeof(m_bSelected));
...
void CCardControl::OnLButtonDown(UINT nFlags, CPoint Point)
{
	...
	if (m_dwCurrentIndex!=0xFFFFFFFF){
		m_bSelected[m_dwCurrentIndex] = TRUE;
	}
...
void CCardControl::OnMouseMove( UINT nFlags, CPoint point ){
	if(MK_LBUTTON == nFlags){
		...
		if (dwCurrentIndex==0xFFFFFFFF) return;

		//设置控件
		m_bSelected[dwCurrentIndex] = TRUE;
