...
//启动服务
bool __cdecl CAttemperEngine::BeginService()
{
	//判断状态
	if (m_bService==true) 
	{
	...
	//启动外挂
	if (m_pIAttemperEngineSink->BeginService(GET_MYSELF_INTERFACE(IUnknownEx))==false)
	{
...
//停止服务
bool __cdecl CAttemperEngine::EndService()
{
	//设置变量
	m_bService=false;
	...
	if (m_pIAttemperEngineSink!=NULL)
	{
		m_pIAttemperEngineSink->EndService(GET_MYSELF_INTERFACE(IUnknownEx));
	}
