...
//启动服务
bool __cdecl CDataBaseEngine::BeginService()
{
	//判断状态
	if (m_bService==true) 
	{
	...
	//启动外挂
	if (m_pIDataBaseSink->BeginService(GET_MYSELF_INTERFACE(IUnknownEx))==false)
	{
...
//停止服务
bool __cdecl CDataBaseEngine::EndService()
{
	//设置变量
	m_bService=false;
	...
	if (m_pIDataBaseSink!=NULL)
	{
		m_pIDataBaseSink->EndService(GET_MYSELF_INTERFACE(IUnknownEx));
	}
