...
typedef CArrayTemplate<CListProcess *> CListProcessArray;
...
public:
	...
	//查找子项
	CListProcess * SearchProcessItem(WORD wKindID);
	//查找子项
...
//进程类型
class CListProcess : public CListItem
{
	//变量定义
public:
	bool							m_bInstall;					//安装标志
	tagGameProcess					m_GameProcess;				//信息结构

	//函数定义
public:
	//构造函数
	CListProcess(CListItem * pParentItem) : CListItem(pParentItem,ItemGenre_Process)
	{
		m_bInstall=false;
		memset(&m_GameProcess,0,sizeof(m_GameProcess));
	}
	//获取数据
	tagGameProcess * GetItemInfo() 
	{ 
...
protected:
	...
	CListProcessArray				m_PtrArrayProcess;				//进程指针
...
public:
	...
	//插入子项
	bool InsertProcessItem(tagGameProcess GameProcess[], WORD wCount);
	//插入子项
...
private:
	...
	//获取图标
	DWORD GetGameImageIndex(CListProcess * pListProcess);
	//获取图标
...
private:
	...
	//获取标题
	LPCTSTR GetGameItemTitle(CListProcess * pListProcess, LPTSTR pszTitle, WORD wBufferSize);
	//获取标题
