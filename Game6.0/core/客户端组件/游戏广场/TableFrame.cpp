...
bool CTableResource::LoadFromFiles(LPCTSTR pszGameName)
{
	try
	{
		...
		{
			m_bDZShowHand=true;
			_snprintf(szResourcePath,sizeof(szResourcePath),TEXT("%s\\Player.BMP"),pszGameName);
...
//配置函数
bool __cdecl CTableFrame::InitTableFrame(WORD wTableCount, WORD wChairCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx)
{
	//效验参数
