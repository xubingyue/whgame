...
bool CServerListManager::InsertProcessItem(tagGameProcess GameProcess[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			pListProcess->m_bInstall=WinFileInfo.OpenWinFile(GameProcess[i].szProcessName);
			m_PtrArrayProcess.Add(pListProcess);
			m_pIServerListSink->OnListItemInserted(pListProcess);
		}
		return true;
	}
	catch (...) { SafeDelete(pListProcess); }
...
DWORD CServerItemView::GetGameImageIndex(CListProcess * pListProcess)
{
	...
	//获取图标
	tagGameProcess * pGameProcess=pListProcess->GetItemInfo();
	DWORD dwImageIndex=GetGameImageIndex(pGameProcess->szProcessName,pGameProcess->wKindID);
...
LPCTSTR CServerItemView::GetGameItemTitle(CListProcess * pListProcess, LPTSTR pszTitle, WORD wBufferSize)
{
	...
	if (m_bShowOnLineCount==true)
	{
		...
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s [ %ld ]"),pGameProcess->szKindName,pGameProcess->dwOnLineCount);
		}
		else
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s [ %ld ] （双击下载）"),pGameProcess->szKindName,pGameProcess->dwOnLineCount);
		}
	}
	else
	{
		if (pListProcess->m_bInstall==true)
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s"),pGameProcess->szKindName);
		}
		else
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s （双击下载）"),pGameProcess->szKindName);
		}
	}
