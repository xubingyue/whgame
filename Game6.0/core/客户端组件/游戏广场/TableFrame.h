...
interface ITableView : IUnknownEx
{
	...
	//绘画函数
	virtual void __cdecl DrawTable(CDC * pDC, CTableResource * pTableResource, bool bShowUser)=NULL;
};
...
interface ITableFrameView : public IUnknownEx
{
	...
	//配置函数
	virtual bool __cdecl InitTableFrame(WORD wTableCount, WORD wPlayerCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx)=NULL;
	//销毁函数
...
public:
	...
	//是否有效
	virtual bool __cdecl IsValid() { return AfxIsValidAddress(this,sizeof(CTableView))?true:false; }
	//接口查询
...
public:
	//绘画函数
	virtual void __cdecl DrawTable(CDC * pDC, CTableResource * pTableResource, bool bShowUser);
};
...
public:
	...
	//配置函数
	virtual bool __cdecl InitTableFrame(WORD wTableCount, WORD wChairCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx);
	//销毁函数
