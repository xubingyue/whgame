...
BOOL CRoomViewItem::OnInitDialog()
{
	...
	m_BrowerAD.Navigate(TEXT("http://game.baidu.com/AD/GameRoomAD.asp"),NULL,NULL,NULL,NULL);
...
void CRoomViewItem::DrawTableFrame(CDC * pDC)
{
	...
	//加载资源
	HDC hDC=pDC->m_hDC;
	CImageHandle ImageTableBLHandle(&m_ImageTableBL);
	CImageHandle ImageTableBMHandle(&m_ImageTableBM);
	CImageHandle ImageTableBRHandle(&m_ImageTableBR);
	CImageHandle ImageTableLMHandle(&m_ImageTableLM);
	CImageHandle ImageTableRMHandle(&m_ImageTableRM);
	CImageHandle ImageTableTLHandle(&m_ImageTableTL);
	CImageHandle ImageTableTMHandle(&m_ImageTableTM);
	CImageHandle ImageTableTRHandle(&m_ImageTableTR);

	//绘画上框
	m_ImageTableTL.BitBlt(hDC,0,0);
	m_ImageTableTM.StretchBlt(hDC,m_ImageTableTL.GetWidth(),0,
		rcVorSplitter.left-m_ImageTableTL.GetWidth()-m_ImageTableTR.GetWidth(),
		m_ImageTableTM.GetHeight());
	m_ImageTableTR.BitBlt(hDC,rcVorSplitter.left-m_ImageTableTR.GetWidth(),0);

	//绘画左右
	for (int nYPos=m_ImageTableTL.GetHeight();nYPos<rcClient.Width();nYPos+=m_ImageTableLM.GetHeight())
	{
		m_ImageTableLM.BitBlt(hDC,0,nYPos);
		m_ImageTableRM.BitBlt(hDC,rcVorSplitter.left-m_ImageTableRM.GetWidth(),nYPos);
	}

	//绘画下框
	m_ImageTableBL.BitBlt(hDC,0,rcClient.Height()-m_ImageTableBL.GetHeight());
	m_ImageTableBM.StretchBlt(hDC,m_ImageTableBL.GetWidth(),rcClient.Height()-m_ImageTableBM.GetHeight(),
		rcVorSplitter.left-m_ImageTableBR.GetWidth(),m_ImageTableBM.GetHeight());
	m_ImageTableBR.BitBlt(hDC,rcVorSplitter.left-m_ImageTableBR.GetWidth(),rcClient.Height()-m_ImageTableBR.GetHeight());
	...
	CRect rcTitle(10,5,rcVorSplitter.left-BUTTON_AREA_WIDTH-8,TABLE_TOP);
...
void CRoomViewItem::DrawLeftViewFrame(CDC * pDC)
{
	...
	HDC hDC=pDC->m_hDC;
	int nXPos=0,nYPos=0;
	int nComminutePos=(rcClient.Height()-145)*3/7+145;
	...
	//绘画上框
	m_ImageLeftViewLT.BitBlt(hDC,rcVorSplitter.right,0);
	for (nXPos=m_ImageLeftViewLT.GetWidth()+rcVorSplitter.right;nXPos<rcClient.Width();nXPos+=m_ImageLeftViewMT.GetWidth())
	{
	...
	{
		m_ImageLeftViewLTM.BitBlt(hDC,rcVorSplitter.right,nYPos);
		m_ImageLeftViewRTM.BitBlt(hDC,nXPos,nYPos);
	}

	//绘画中间
	m_ImageLeftViewLM.BitBlt(hDC,rcVorSplitter.right,nComminutePos-m_ImageLeftViewLM.GetHeight());
	for (nXPos=m_ImageLeftViewLM.GetWidth()+rcVorSplitter.right;nXPos<rcClient.Width();nXPos+=m_ImageLeftViewMM.GetWidth())
	{
	...
	{
		m_ImageLeftViewLBM.BitBlt(hDC,rcVorSplitter.right,nYPos);
		m_ImageLeftViewRBM.BitBlt(hDC,nXPos,nYPos);
	}

	//绘画下框
	nYPos=rcClient.Height()-m_ImageLeftViewMB.GetHeight();
	m_ImageLeftViewLB.BitBlt(hDC,rcVorSplitter.right,rcClient.Height()-m_ImageLeftViewLB.GetHeight());
	for (nXPos=rcVorSplitter.right+m_ImageLeftViewLB.GetWidth();nXPos<rcClient.Width()-m_ImageLeftViewRB.GetWidth();nXPos+=m_ImageLeftViewMB.GetWidth())
	{
...
void CRoomViewItem::RectifyControl(int nWidth, int nHeight)
{
	...
	const int nSplitterWidth=6,nComminutePos=(nHeight-145)*3/7+145;
	...
	//设置拆分范围
	int nVorLessPos=nWidth*3/5,nVorMaxPos=nWidth*75/100;
	...
	//桌子区域
	DeferWindowPos(hDwp,m_TableFrame,NULL,TABLE_LEFT,TABLE_TOP,rcVorSplitter.left-TABLE_RIGHT-TABLE_LEFT,
		nHeight-TABLE_BOTTOM-TABLE_TOP,uFlags);

	//列表区域
	DeferWindowPos(hDwp,m_UserListView,NULL,rcVorSplitter.right+LIST_LEFT,LIST_TOP,nWidth-rcVorSplitter.right-LIST_LEFT-LIST_RIGHT,
		nComminutePos-LIST_TOP-LIST_BOTTOM,uFlags);

	//广告区域
	DeferWindowPos(hDwp,m_BrowerAD,NULL,rcVorSplitter.right+LIST_LEFT,7,nWidth-rcVorSplitter.right-LIST_LEFT-LIST_RIGHT,65,uFlags);

	//聊天区域
	DeferWindowPos(hDwp,m_ChatMessage,NULL,rcVorSplitter.right+CHAT_LEFT,nComminutePos+CHAT_TOP,nWidth-rcVorSplitter.right-CHAT_RIGHT-CHAT_LEFT,
	...
	{
		DeferWindowPos(hDwp,pButtonArray[i]->m_hWnd,NULL,rcVorSplitter.right+CHAT_LEFT+(rcButton.Width()+5)*i,nHeight-61,rcButton.Width(),rcButton.Height(),uFlags);
	}
	...
	DeferWindowPos(hDwp,m_btQuitRoom,NULL,rcVorSplitter.left-rcButton.Width()-TABLE_RIGHT-12,5,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btAutoSit,NULL,rcVorSplitter.left-rcButton.Width()*2-TABLE_RIGHT-20,5,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btFindUser,NULL,rcVorSplitter.left-rcButton.Width()*3-TABLE_RIGHT-28,5,rcButton.Width(),rcButton.Height(),uFlags);
...
bool CRoomViewItem::SendTableUser(IUserItem * pIUserItem, CIPCSendCopyData * pSendCopyData)
{
	...
	SendPacketHelper.AddPacket(pUserData->szUnderWrite,CountString(pUserData->szUnderWrite),DTP_UNDER_WRITE);
...
bool CRoomViewItem::OnSocketMainGameFrame(CMD_Command Command, void * pBuffer, WORD wDataSize, IClientSocket * pIClientSocke)
{
	...
	if (m_hWndChannel==NULL)
	{
		...
		if ((pUserData->wTableID!=INVALID_TABLE)&&(pUserData->wChairID!=INVALID_CHAIR))
		{
			...
			CopyMemory(&cbBuffer[sizeof(Command)],pBuffer,wDataSize);

			//插入数据
			m_PacketDataStorage.AddData(0,cbBuffer,sizeof(Command)+wDataSize);
		}
	}
	else
	{
	...
	case IPC_SUB_IPC_CLIENT_CONNECT:	//连接消息
		{
			...
			if ((m_wPacketTableID!=INVALID_TABLE)&&(m_wPacketChairID!=INVALID_CHAIR))
			{
				...
				do
				{
					...
					if (m_PacketDataStorage.GetData(DataHead,cbBuffer,sizeof(cbBuffer))==false) break;
