...
//运行函数
bool CDownLoadThread::RepetitionRun()
{
	try
	{
		if (m_bPreparative==true)
		{
			...
			else 
			{
				...
				if (MoveFile(m_szTempFile,m_szLocalFile)==FALSE) throw enDownLoadResult_CreateFileFails;
...
//关闭事件
bool CDownLoadThread::OnThreadStopEvent()
{
	DownLoadCleanUp();
	return true;
}
...
bool __cdecl CDownLoadService::SetDownLoadServiceSink(IUnknownEx * pIUnknownEx)
{
	...
	m_pIDownLoadServiceSink=GET_OBJECTPTR_INTERFACE(pIUnknownEx,IDownLoadServiceSink);
